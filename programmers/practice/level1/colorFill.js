...
즉, 롤러의 좌우측 끝을 구역의 경계선 혹은 벽의 좌우측 끝부분에 맞춘 후
 롤러를 위아래로 움직이면서 벽을 칠합니다. 현재 페인트를 칠하는 구역들을 완전히 칠한 후
 벽에서 롤러를 떼며, 이를 벽을 한 번 칠했다고 정의합니다.
...
function solution(n, m, section) {
  let answer = 1; // 무조건 한번은 칠하기 때문에 1로 초기화
  let pivot = section[0]; // 기준점으로 초기화
  section.forEach((s) => {
    if (pivot + m - 1 < s) {
      pivot = s; // 현재 구간에서 칠할 수 없다면 위치 변경
      answer += 1; // 회수 증가
    }
  });
  return answer;
